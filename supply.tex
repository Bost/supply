...
There is a one-to-one correspondence between supplies $s$ of $\pp$ in $\cat{C}$ (as in \cref{def.supply}) and strong monoidal functors $s'\colon\pp\to\smf(\mob{\cat{C}},\cat{C})$ such that 
\begin{enumerate}[label=(\roman*)]
	\item $m\mapsto\inc\tpow{m}$ for each $m\in\nn$ and
	\item each strongator $s'(m)\otimes s'(n)\to s'(m+n)$ is the unique coherence isomorphism.
\end{enumerate}
...
\end{equation}
There is no data in any of the above, nor in the strongators $s'(m)\otimes s'(n)\to s'(m+n)$ by condition (ii). The data inherent in an $s'$ comes in terms of how it applies to morphisms in $\pp$.
...
Let $\cat{C}$ be a symmetric strict monoidal category. There is a one-to-one correspondence between supplies $s$ of $\pp$ in $\cat{C}$ and strict monoidal functors $s'\colon\pp\to\smf(\mob{\cat{C}},\cat{C})$ such that $1 \mapsto \inc$.
\end{corollary}
