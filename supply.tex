...
The coherence theorem itself states that that every symmetric monoidal category is equivalent to a symmetric strict monoidal category.

\begin{definition} \label{def.objectwise_free}
	Let $\cat{C}$ be a strict monoidal small category; note that its objects form a monoid. We say that $\cat{C}$ is an \emph{objectwise-free} monoidal category if this monoid is isomorphic to the free monoid on some set $G$. We call the elements of $G$ the \emph{generators} of the objectwise-free monoidal category.
\end{definition}
...
\begin{definition} \label{def.semistrict}
  Let $\pp$ be an objectwise-free symmetric monoidal category with generating set $G$, and let $\cat{C}$ be any symmetric monoidal category. Note that the objects of $\pp$ may be uniquely written as $g_1 \otimes g_2 \otimes \dots \otimes g_n$ where each $g_i \in G$. 
  
  A \emph{semistrict} monoidal functor $F\colon \pp \to \cat{C}$ is a strong monoidal functor that sends each object $g_1 \otimes g_2 \otimes \dots \otimes g_n$ in $\pp$ to $(((I \otimes F(g_1)) \otimes F(g_2)) \otimes \dots ) \otimes F(g_n)$.
\end{definition}

\begin{proposition} \label{thm.sstfae}
  A strong monoidal functor $F\colon \pp \to \cat{C}$ is semistrict if and only if it factors 
  \[
    \begin{tikzcd}[column sep=large]
      \pp \ar[r,dashed] \ar[dr,"F"'] & \strict{\cat{C}} \ar[d]\\
      & \cat{C}
    \end{tikzcd}
  \]
\end{proposition}
\begin{proof}
  Let $F$ be a semistrict monoidal functor. We define $\strict{F}\colon \pp \to \strict{\cat{C}}$ by sending $g_1 \otimes g_2 \otimes \dots \otimes g_n$ to the object $[F(g_1),\dots,F(g_n)]$, and 

  Conversely, given a functor $\pp \to \strict{\cat{C}}$, we simply compose with $s$ to obtain a functor $F\colon \pp \to \cat{C}$. This functor is the composite of two strong monoidal functors, and hence strong monoidal itself, and is indeed semistrict by the definition of $s$.
\end{proof}
...
Recall that a prop $\pp$ is a symmetric strict monoidal category whose monoid of objects is $(\nn,0,+)$. We denote its objects by $m$, $n$, etc. Recall the definition of semistrict monoidal functors from \cref{def.semistrict}.

\begin{definition}[Supply]\label{def.supply}
Let $\pp$ be a prop and $\cat{C}$ a symmetric monoidal category. A \emph{supply of $\pp$ in $\cat{C}$} consists of a semistrict monoidal functor $s_c\colon\pp\to\cat{C}$ for each object $c\in\cat{C}$, such that $s_c(1)=c$ and such that the following diagrams commute for every $c,d\in\cat{C}$:
...
Let $\nn$ be the discrete prop. For any monoidal category $\cat{C}$ and object $c\in\cat{C}$, there is a unique semistrict monoidal functor $s_c\colon\nn\to\cat{C}$ sending $1\mapsto c$, since to be semistrict it must send $m\mapsto c\tpow{m}$. There is nothing more to check for $s$ to be a supply, since $\nn$ is discrete, and in fact every morphism in $\cat{C}$ is an $s$-homomorphism. 
...
\end{enumerate}
Thus $\mob{\cat{C}}$ inherits a symmetric monoidal structure, and we refer to it as the \emph{symmetric monoidal category of $\cat{C}$-objects}. There is an identity-on-objects faithful functor $\inc\colon\mob{\cat{C}}\to\cat{C}$. 
\end{definition}
...
\todo{Break off \cref{cor.coherence_isos_homos} as a lemma}
