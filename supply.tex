...
%============ Abstract ============%
\begin{abstract}
It is common to encounter symmetric monoidal (SM) categories $C$ for which every object is equipped with an algebraic structure, in a way that is compatible with its monoidal unit and product. We define this formally and say that $C$ \emph{supplies} this structure. For example, the category of relations has two SM structures, and with respect to either one it supplies comonoids. A hypergraph category is defined to be a SM category which supplies frobenius monoids. 

We prove several facts about the notion of supply, e.g.\ that the coherence isomorphisms---associators and unitors---are automatically homomorphisms for any supply, and so are the strong coherence isomorphisms for any SM functor preserving supply. Along the way, we also prove that the 2-category of SM categories, strong SM functors, and monoidal natural transformations has biproducts, a fact which does not appear to be well-known and which we had difficulty finding in the literature.
\end{abstract}
...
In \cref{sec.supply} we formally define supply and give several examples. In \cref{sec.pres_supply} we formally define what it means for a functor to preserve supply and again give several examples.
...
Let $s$ be a supply of $\pp$ in $\cat{C}$ and $t$ a supply of $\pp$ in $\cat{D}$, and suppose that $(F,\varphi)\colon\cat{C}\to\cat{D}$ is a strong monoidal functor preserving supply. Then the strongators $\varphi$ are $t$-homomorphisms, i.e.\ the following diagram commutes for each morphism $\mu\colon m\to n$ in $\pp$ and objects $c,c'\in\cat{C}$:
\[
\begin{tikzcd}[column sep=50pt]
	(Fc\otimes Fc')\tpow{m}
