...
\end{tikzpicture}
\]
The point is that every object in $\rel$ has this comonoid structure, and the operations are coherent with $I,\otimes$. In this situation we will say that $\rel$ \emph{supplies comonoids}.
...
We also provide some basic theory of supply. For example, given a supply of $\pp$ in $\cat{C}$ and a prop functor $\pp'\to\pp$, one obtains a supply of $\pp'$ in $\cat{C}$. We show that if $\cat{C}$ and $\cat{D}$ both supply $\pp$ then so does their biproduct $\cat{C}\oplus\cat{D}$, and that the projections and coprojections preserve supplies. It does not appear to be sufficiently well-known that the 2-category of symmetric monoidal categories has (strong) biproducts, so we prove that as well.

We also discuss what it means for various maps in $(\cat{C},I,\otimes)$ to be \emph{homomorphisms} for the supplied structure. For example, it is well-known that if $\cat{C}$ is Cartesian monoidal then it supplies comonoids and every morphism $f\colon c\to d$ is a comonoid homomorphism, in the sense that the following diagrams commute:
\[
\begin{tikzcd}[column sep=small]
...
\cref{thm.smc_biprod,prop.biprod_smf_strict} are not necessary for the main thrust of this note, so we will save their proofs for later; see \cref{chap.proofs}, pages \pageref{page.smc_biprod} and \pageref{page.biprod_smf_strict}. However, they seem important to us, and not sufficiently well known. 
...
In \cref{sec.supply} we formally define what it means for a symmetric monoidal category to supply an algebraic structure coming from a prop, and we give several examples. In \cref{sec.pres_supply} we formally define what it means for a strong symmetric monoidal functor to preserve supply and again give several examples.
...
\end{tikzcd}
\end{equation}
where the $\sigma$'s are as in \cref{eqn.symmetry}. We say that $f\colon c\to d$ is an \emph{$s$-homomorphism} if the following diagram commutes for all $\mu\colon m\to n$ in $\pp$:
