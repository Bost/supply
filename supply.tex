...
%======== Chapter ========%
\chapter{Objectwise free SMCs and semistrict functors}

\todo{Introduce this section; talk about how strictification yields objectwise free things.}
...
Let $\pp$ be a prop and $\cat{C}$ a symmetric monoidal category. A \emph{supply of $\pp$ in $\cat{C}$} consists of a strong monoidal functor $s_c\colon\pp\to\cat{C}$ for each object $c\in\cat{C}$, such that $s_c(m)=c\tpow{m}$ for all $m$, and such that the following diagrams commute for every $c,d\in\cat{C}$:
