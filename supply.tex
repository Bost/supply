...
% biblatex %
  \addbibresource{Library20190702.bib} 
...
We thank Dmitry Vagner for interesting and useful conversations and Tobias Fritz for catching an error in an earlier draft and for suggesting the strictification theorem \cref{thm.supply_strictification}. We also acknowledge support from AFOSR grants FA9550-17-1-0058 and FA9550-19-1-0113.
...
\begin{enumerate}
	...
	\item the strongator $s_c(m)\otimes s_c(n)\to s_c(m+n)$ is the unique coherence isomorphism for each $m,n\in\nn$ and
...
For any $k\in\pp$ let $s_k\colon\pp\to\pp$ act on objects by $s_k(n)=k\cdot n$; this is strict because $s_k(m+n)=k\cdot(m+n)=(k\cdot m)+(k\cdot n)$. Given $\mu\colon m\to n$ in $\pp$, define $s_k(\mu)\colon k\cdot m\to k\cdot n$ by conjugating with the symmetries and applying $\mu$, on each of the $k$ pieces:
...
There is a one-to-one correspondence between supplies $s$ as in \cref{def.supply} and strong monoidal functors $s'\colon\pp\to\smf(\mob{\cat{C}},\cat{C})$ satisfying
\begin{enumerate}
	\item $m\mapsto\inc\tpow{m}$ for each $m\in\nn$ and
	\item the strongator $s'(m)\otimes s'(n)\to s'(m+n)$ is the unique coherence isomorphism.
\end{enumerate}
\end{theorem}\begin{proof}
We first unpack what it means to have $s'\colon\pp\to\smf(\mob{\cat{C}},\cat{C})$ satisfying the above two conditions. For any $m\in\nn$, the functor $s'(m)$ acts on objects in $\mob{\cat{C}}$ by $c\mapsto c\tpow{m}$ and similarly for morphisms, and its strongator isomorphisms for any $c,d\in\cat{C}$ are given by symmetry isomorphisms $\sigma$:
...
\end{equation}
There is no data in any of the above, nor in the strongators $s'(m)\otimes s'(n)\to s'(m+n)$ by condition 2. The data inherent in an $s'$ comes in terms of how it applies to morphisms in $\pp$.

To a morphism $\mu\colon m\to n$ in $\pp$, the functor $s'$ provides a monoidal natural transformation $s'(\mu)\colon s'(m)\to s'(n)$. This means that for each object $c\in\cat{C}$, we have a morphism $s'(\mu)_c\colon c\tpow{m}\to c\tpow{n}$ natural in $c\in\mob{\cat{C}}$ and commuting with composition and monoidal product in $\pp$: given $\nu\colon n\to p$ and $\mu'\colon m'\to n'$, the following equations hold
\begin{equation}\label{eqn.supply_composition}
  s'(\mu\cp\nu)_c=s'(\mu)_c\cp s'(\nu)_c
  \qqand
	s'(\mu+\mu')_c=s'(\mu)_c\otimes s'(\mu')_c.
\end{equation}
The fact that the natural transformation $s'(\mu)\colon s'(m)\to s'(n)$ is monoidal means the following diagrams commute for any $c,d\in\cat{C}$:
...
Because of the one-to-one correspondence \cref{thm.supply_v2}, we often elide the difference between the supply $s$ and the strong monoidal functor $s'$. 
...
\begin{enumerate}
	...
	\item the strongator $F(m)\otimes F(n)\to F(m+n)$ is the unique coherence isomorphism.
\end{enumerate}
\end{lemma}
\begin{proof}
...
\begin{tikzcd}[column sep=90pt]
	{\big[[c_1,\Ldots{m},c_1],\ldots,[c_k,\Ldots{m}, c_k]\big]}
		\ar[r, "{\left[s_{[c_1]}(\mu),\ldots, s_{[c_k]}(\mu)\right]}"]&
	{\big[[c_1,\Ldots{n},c_1],\ldots,[c_k,\Ldots{n}, c_k]\big]}
		\ar[d, "\sigma"]\\
	{\big[[c_1,\ldots, c_k],\Ldots{m},[c_1,\ldots,c_k]\big]}
		\ar[u, "\sigma"]\ar[r, dashed, "\strict{s}_c(\mu)"']&
	{\big[[c_1,\ldots, c_k],\Ldots{n},[c_1,\ldots,c_k]\big]}
\end{tikzcd}
\]
...
\cref{prop.strict_equiv_pres_supply} follows easily from the proof of \cref{thm.supply_strictification}.
\begin{proposition}\label{prop.strict_equiv_pres_supply}
