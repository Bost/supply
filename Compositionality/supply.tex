...
Let $\cat{C}$ be a symmetric strict monoidal category. There is a one-to-one correspondence between supplies $s$ of $\pp$ in $\cat{C}$ and strict monoidal functors $\tilde{s}\colon\pp\to\smf(\mob{\cat{C}},\cat{C})$ such that $1 \mapsto \inc$. Moreover, for each $n\in\pp$, the functor $\tilde{s}\colon\mob{\cat{C}}\to\cat{C}$ is strict.\todo{Check}
\end{corollary}
