...
\begin{example}\label{ex.strict_mon_cat_obs}
If $\cat{C}$ is a strict monoidal category then $\mob{\cat{C}}=\ob(\cat{C})$ is discrete.
\end{example}
...
\begin{enumerate}[label=(\roman*)]
	\item $\tilde{s}(m)=\inc\tpow{m}$ for each $m\in\nn$, and
...
Let $\cat{C}$ be a symmetric strict monoidal category. There is a one-to-one correspondence between supplies $s$ of $\pp$ in $\cat{C}$ and strict monoidal functors $\tilde{s}\colon\pp\to\smf^{st}(\ob(\cat{C}),\cat{C})$ such that $1 \mapsto \inc$.
\end{corollary}
\begin{proof}
By \cref{ex.strict_mon_cat_obs}, $\mob{\cat{C}}=\ob(\cat{C})$, and all the coherence maps in $\cat{C}$ are identities, so by \cref{thm.supply_v2} we have a one-to-one correspondence between supplies $s$ and strict monoidal functors 
\end{proof}
...
\begin{corollary}
Let $\pp$ and $\qq$ be props. There is a bijection between the set of supplies of $\pp$ in $\qq$ and the set of prop functors $\pp\to\qq$.
\end{corollary}
\begin{proof}
\todo{Check}
By \cref{ex.strict_mon_cat_obs}, $\qq_0=\nn$. In the context of \cref{thm.supply_v2}, \cref{prop.p_supplies_itself} provides a strict functor $A\colon\qq\to\smf^{st}(\nn,\qq)$, where this notation means that $A(n)\colon\nn\to\qq$ is strict for every $n\in\qq$; see \cref{cor.strict_supply_v2}. Conversely, there is a strict monoidal functor $B\colon\smf^{st}(\nn,\qq)\to\qq$ sending $B(F)\coloneqq F(1)$. Each of the round-trip composites $(A\cp B)\colon\qq\to\qq$ and $(B\cp A)\colon\smf^{st}(\nn,\qq)\to\qq\to\smf^{st}(\nn,\qq)$ is the identity.

The required bijection is now obvious. Suppose given a prop functor $\pp\to\qq$, we compose with $A$ to obtain a supply $\pp\to\smf^{st}(\nn,\qq)$. Given a supply $\pp\to\smf(\nn,\qq)$, it is in fact of the form $\pp\to\smf^{st}(\nn,\qq)$ by \cref{cor.strict_supply_v2}. We compose with $B$ to obtain a prop functor $\pp\to\qq$. These constructions are mutually inverse.
\end{proof}
